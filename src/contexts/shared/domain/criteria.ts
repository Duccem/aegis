...
export class Criteria {
  ...
  offset(offset: number): Criteria {
    this.pagination = this.hasPagination()
  ...
  static withFilters(filters: Filter[]): Criteria {
    return new Criteria({ filters, type: FilterType.AND });
  }
  ...
  buildMeta(total: number): Meta {
    return {
      total,
      page: this.pagination?.offset
        ? Math.floor(this.pagination.offset / (this.pagination.limit || 10)) + 1
        : 1,
      size: this.pagination?.limit || 10,
      pages: Math.ceil(total / (this.pagination?.limit || 10)),
    };
  }
}
