...
export default function InputMultipleImages({
  ...
  selectFileAction?: (file: File[]) => void;
  value?: string[] | null;
}) {
  const maxSizeMB = 5;
  ...
  useEffect(() => {
    if (files.length > 0) {
      const fileUrls = files.map((file) =>
        file.file instanceof File ? URL.createObjectURL(file.file) : file.file.url,
      );
      const filesData: File[] = files.map((file) =>
        file.file instanceof File ? file.file : new File([], file.file.url),
      );
      changeAction?.(fileUrls);
      selectFileAction?.(filesData);
    }
  }, [files]);
